...
### High
h1 - Potential invalid mix data for short tracks when appending to an existing mix: fixed 5s/2s crossfade math can yield negative `attachTo`/envelope times if `trackInfo.duration < 5000ms`, which can propagate into playback/export and timeline math. `UI/CreateMixDialogComponent.cpp:251`
h2 - Use-after-free risk from async UI callbacks in long-running tasks: background thread calls `MessageManager::callAsync` capturing raw `this` without a `Component::SafePointer`, so closing the dialog while a task is running can crash. `UI/TaskDialog.cpp:185`
