...
namespace jucyaudio
{
    ...
    namespace database
    {
        ...
        AlbumsNode::AlbumsNode(
            INavigationNode *root, const std::string &name, std::string_view typeNameForSingleObject, std::string_view typeNameForMultipleObjects)
                ...
                if (!m_searchTerms.empty())
                {
                    ...
                    for (const auto &album : m_albums)
                    {
                        ...
                        for (const auto &term : m_searchTerms)
                        {
                            ...
                            if (lowerArtist.find(lowerTerm) != std::string::npos || lowerTitle.find(lowerTerm) != std::string::npos)
                            {
                                matches = true;
                                break;
                            }
