...
namespace jucyaudio::ui
{
    ...
    EqualizerComponent::EqualizerComponent()
    {
        ...
        m_preampSlider.onValueChange = [this]()
        {
            notifySettingsChanged();
        };
    ...
    void EqualizerComponent::setupBandControls(size_t bandIndex)
    {
        ...
        band.gainSlider.onValueChange = [this]()
        {
            notifySettingsChanged();
        };
        ...
        band.qSlider.onValueChange = [this]()
        {
            notifySettingsChanged();
        };
    ...
    void EqualizerComponent::showDeleteConfirmation()
    {
        ...
        if (presetId < 0)
        {
            juce::AlertWindow::showMessageBoxAsync(juce::AlertWindow::WarningIcon, "No Preset Selected", "Please select a preset to delete.");
            return;
        }
        ...
        if (it != m_presets.end())
        {
            if (!it->isDeletable)
            {
                juce::AlertWindow::showMessageBoxAsync(juce::AlertWindow::WarningIcon, "Cannot Delete", "System presets cannot be deleted.");
                return;
            }

            const auto result = juce::AlertWindow::showYesNoCancelBox(
                juce::AlertWindow::QuestionIcon, "Delete Preset", "Are you sure you want to delete the preset \"" + it->name + "\"?", "Delete", "Cancel", "");
    ...
    void EqualizerComponent::paint(juce::Graphics &g)
    {
        ...
        g.drawLine(0, static_cast<float>(getHeight() - 60), static_cast<float>(getWidth()), static_cast<float>(getHeight() - 60), 1.0f);

        // Draw frequency labels using theme color
