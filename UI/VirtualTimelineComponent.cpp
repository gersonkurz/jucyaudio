...
namespace jucyaudio::ui
{
    ...
    //==============================================================================
    VirtualTimelineComponent::VirtualTimelineComponent(juce::AudioFormatManager &formatManager, juce::AudioThumbnailCache &thumbnailCache)
    ...
    void VirtualTimelineComponent::mouseDown(const juce::MouseEvent &event)
    {
        ...
        if (hitInfo.track)
        {
            ...
            if (hitInfo.type == HitTestResult::AttachFrom || hitInfo.type == HitTestResult::AttachTo || hitInfo.type == HitTestResult::EnvelopePoint ||
                hitInfo.type == HitTestResult::CueStart || hitInfo.type == HitTestResult::CueEnd)
            {
                ...
                if (hitInfo.type == HitTestResult::CueStart)
                {
                    ...
                    {
                        dragState_.originalTime = std::chrono::duration<double>(hitInfo.track->mixTrack->cueStart).count();
                    }
                }
                else if (hitInfo.type == HitTestResult::CueEnd)
                {
                    ...
                    {
                        dragState_.originalTime =
                            std::chrono::duration<double>(hitInfo.track->mixTrack->getCueEndActual(hitInfo.track->trackInfo->duration)).count();
                    }
                }
                else if (hitInfo.type == HitTestResult::AttachFrom)
                {
                    ...
                    if (hitInfo.track->mixTrack)
                    {
                        dragState_.originalTime = std::chrono::duration<double>(hitInfo.track->mixTrack->attachFrom).count();
                    }
                }
                else if (hitInfo.type == HitTestResult::AttachTo)
                {
                    ...
                    if (hitInfo.track->mixTrack)
                    {
                        dragState_.originalTime = std::chrono::duration<double>(hitInfo.track->mixTrack->attachTo).count();
                    }
                }
                else if (hitInfo.type == HitTestResult::EnvelopePoint)
                {
                    ...
                    {
                        dragState_.originalTime = std::chrono::duration<double>(hitInfo.track->mixTrack->envelopePoints[hitInfo.pointIndex].time).count();
                    }
                }
            }
        ...
        if (dragState_.isDragging)
        {
            ...
            if (track && track->mixTrack)
            {
                ...
                if (dragState_.dragType == DragState::DragType::CueStart)
                {
                    ...
                    track->mixTrack->cueStart = jucyaudio::Duration_t(static_cast<int64_t>(clampedCueStart * 1000));

                    // Update component start time and effective duration
                    track->componentStartTime = track->audioStartTime + track->mixTrack->cueStart;
                    track->effectiveDuration = std::chrono::duration<double>(track->mixTrack->getEffectiveDuration(track->trackInfo->duration)).count();
                ...
                else if (dragState_.dragType == DragState::DragType::CueEnd)
                {
                    ...
                    // Update the cue end
                    track->mixTrack->cueEnd = jucyaudio::Duration_t(static_cast<int64_t>(clampedCueEnd * 1000));
                ...
                else if (dragState_.dragType == DragState::DragType::AttachFrom)
                {
                    ...
                    const double maxTime = std::chrono::duration<double>(track->mixTrack->getCueEndActual(track->trackInfo->duration)).count();
                    ...
                    track->mixTrack->attachFrom = jucyaudio::Duration_t(static_cast<int64_t>(dragState_.currentTime * 1000));
                }
                else if (dragState_.dragType == DragState::DragType::AttachTo)
                {
                    ...
                    // Update the attach point (temporarily for preview)
                    // Convert seconds to milliseconds (Duration_t is std::chrono::milliseconds)
                    track->mixTrack->attachTo = jucyaudio::Duration_t(static_cast<int64_t>(dragState_.currentTime * 1000));
                }
                else if (dragState_.dragType == DragState::DragType::EnvelopePoint)
                {
                    // Update envelope point time and volume
                    if (dragState_.draggedPointIndex >= 0 && dragState_.draggedPointIndex < static_cast<int>(track->mixTrack->envelopePoints.size()))
                    {
                        ...
                        // Convert seconds to milliseconds (Duration_t is std::chrono::milliseconds)
                        track->mixTrack->envelopePoints[dragState_.draggedPointIndex].time = jucyaudio::Duration_t(static_cast<int64_t>(clampedTime * 1000));

                        // Convert normalized volume to Volume_t (integer where 1000 = 1.0)
                        track->mixTrack->envelopePoints[dragState_.draggedPointIndex].volume = static_cast<jucyaudio::Volume_t>(newVolume * 1000);
                    }
                }
            }
    ...
    void VirtualTimelineComponent::mouseUp(const juce::MouseEvent &event)
    {
        if (dragState_.isDragging)
        {
            ...
            if (modifiedTrack && modifiedTrack->mixTrack)
            {
                ...
                if (dragState_.dragType == DragState::DragType::CueStart || dragState_.dragType == DragState::DragType::CueEnd)
                {
                    ...
                    {
                        onCuePointsChanged(modifiedTrack->mixTrack->orderInMix, modifiedTrack->mixTrack->cueStart, modifiedTrack->mixTrack->cueEnd);
                    }
                }
                else if (dragState_.dragType == DragState::DragType::AttachFrom || dragState_.dragType == DragState::DragType::AttachTo)
                {
    ...
    void VirtualTimelineComponent::mouseMove(const juce::MouseEvent &event)
    {
        ...
        else if (hitInfo.type == HitTestResult::AttachFrom || hitInfo.type == HitTestResult::AttachTo || hitInfo.type == HitTestResult::EnvelopePoint)
        {
    ...
    void VirtualTimelineComponent::loadMixProject(audio::MixProjectLoader *loader)
    {
        ...
        for (size_t i = 0; i < mixTracks.size(); ++i)
        {
            ...
            if (data.trackInfo)
            {
                ...
                data.effectiveDuration = std::chrono::duration<double>(mixTrack.getEffectiveDuration(data.trackInfo->duration)).count();
            }
            else
            {
    ...
    void VirtualTimelineComponent::showContextMenu(juce::Point<int> position)
    {
        ...
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(this).withTargetScreenArea(juce::Rectangle<int>(screenPos.x, screenPos.y, 1, 1)),
            [this](int result)
            {
    ...
    void VirtualTimelineComponent::pasteTracksAfterSelection()
    {
        if (clipboardTracks_.empty() || selectedTracks_.empty())
        {
            spdlog::warn("[PASTE] Cannot paste - clipboard empty: {}, no selection: {}", clipboardTracks_.empty(), selectedTracks_.empty());
            return;
        }
    ...
    void VirtualTimelineComponent::showGainAdjustment()
    {
        ...
        {
        public:
            GainAdjustmentComponent(float initialGain, std::function<void(float, bool)> onGainChanged) // bool = save to database
                : 
                  originalGain(initialGain),
                  gainChangedCallback(onGainChanged)
            {
                // Setup slider
                ...
                // Setup label
                gainLabel.setText(juce::String(initialGain, 2) + "x (" + juce::String(20.0f * std::log10(initialGain), 1) + " dB)", juce::dontSendNotification);
    ...
    void VirtualTimelineComponent::showTrackProperties()
    {
        ...
        if (cueStartSecs < 0)
            info << " (adds " << -cueStartSecs << "s silence before)";
        else if (cueStartSecs > 0)
            info << " (skips first " << cueStartSecs << "s)";
        info << "\n";

        info << "Cue End: " << cueEndSecs << " seconds";
        if (cueEndSecs > 0)
            info << " (adds " << cueEndSecs << "s silence after)";
        else if (cueEndSecs < 0)
            info << " (cuts last " << -cueEndSecs << "s)";
        info << "\n";
        ...
        if (!selectedTrack->mixTrack->envelopePoints.empty())
        {
            for (size_t i = 0; i < selectedTrack->mixTrack->envelopePoints.size(); ++i)
            {
                ...
                info << "  Point " << (i + 1) << ": time=" << std::chrono::duration<double>(point.time).count() << "s, "
        ...
        for (auto &track : tracks_)
        {
            if (track.bounds.contains(point))
            {
                ...
                if (track.mixTrack)
                {
                    ...
                    // AttachFrom point
                    const double attachFromTime = std::chrono::duration<double>(track.audioStartTime + track.mixTrack->attachFrom).count();
                    const int attachFromX =
                        track.bounds.getX() +
                        static_cast<int>((attachFromTime - std::chrono::duration<double>(track.componentStartTime).count()) * pixelsPerSecond_);
                    ...
                    // AttachTo point
                    const double attachToTime = std::chrono::duration<double>(track.audioStartTime + track.mixTrack->attachTo).count();
                    const int attachToX = track.bounds.getX() +
                                          static_cast<int>((attachToTime - std::chrono::duration<double>(track.componentStartTime).count()) * pixelsPerSecond_);
                    ...
                    {
                        const double pointTimeInComponent = std::chrono::duration<double>(track.audioStartTime + envPoint.time).count();
                        const int x =
                            track.bounds.getX() +
                            static_cast<int>((pointTimeInComponent - std::chrono::duration<double>(track.componentStartTime).count()) * pixelsPerSecond_);

                        const float normalizedVolume = static_cast<float>(envPoint.volume) / 1000.0f;
                        const int y = track.waveformBounds.getY() + static_cast<int>((1.0f - normalizedVolume) * track.waveformBounds.getHeight());
    ...
    juce::Rectangle<int> VirtualTimelineComponent::getVisibleArea() const
    {
        ...
        return juce::Rectangle<int>(0, viewportBounds_.getY(), getWidth(), viewportBounds_.getHeight());
    }
    ...
    void VirtualTimelineComponent::paintTrack(juce::Graphics &g, const TrackRenderData &track)
    {
        ...
        if (track.thumbnail && track.thumbnail->isFullyLoaded())
        {
            ...
            if (!trackVisibleBounds.isEmpty())
            {
                    ...
                    for (const auto &key : tileKeys)
                    {
                        if (auto tile = tileCache_->getTile(key))
                        {
                            if (tile->isReady)
                            {
                                ...
                                // Draw the tile image
                                g.setOpacity(1.0f);
                                g.drawImage(tile->image, destRect.toFloat(), juce::RectanglePlacement::stretchToFit);
                            }
                            else
                            {
                                allTilesReady = false;
                ...
                if (!allTilesReady && tilesReady == 0)
                {
                    ...
                    {
                        thumbnailStartTime = std::chrono::duration<double>(std::max(jucyaudio::Duration_t{0}, track.mixTrack->cueStart)).count();
        ...
        const auto labelBounds = juce::Rectangle<int>(track.bounds.getX() + 4, track.bounds.getY() + 2, track.bounds.getWidth() - 8, 20);
        ...
        if (track.mixTrack && !track.mixTrack->envelopePoints.empty())
        {
            ...
            for (const auto &point : track.mixTrack->envelopePoints)
            {
                ...
                const double pointTimeInComponent = std::chrono::duration<double>(track.audioStartTime + point.time).count();
                const int x = track.bounds.getX() +
                              static_cast<int>((pointTimeInComponent - std::chrono::duration<double>(track.componentStartTime).count()) * pixelsPerSecond_);
            ...
            for (const auto &point : track.mixTrack->envelopePoints)
            {
                ...
                g.drawEllipse(static_cast<float>(x) - pointRadius, static_cast<float>(y) - pointRadius, pointRadius * 2.0f, pointRadius * 2.0f, 2.0f);
                g.setColour(juce::Colours::white);
                g.fillEllipse(static_cast<float>(x) - pointRadius, static_cast<float>(y) - pointRadius, pointRadius * 2.0f, pointRadius * 2.0f);
            }
        }
        ...
        if (track.mixTrack)
        {
            ...
            {
                const double attachFromTime = std::chrono::duration<double>(track.audioStartTime + track.mixTrack->attachFrom).count();
                const int attachFromX = track.bounds.getX() +
                                        static_cast<int>((attachFromTime - std::chrono::duration<double>(track.componentStartTime).count()) * pixelsPerSecond_);

                // Draw attach point as a vertical line with diamond
                g.drawVerticalLine(attachFromX, static_cast<float>(track.bounds.getY()), static_cast<float>(track.bounds.getBottom()));

                // Draw diamond marker
                juce::Path diamond;
                diamond.addQuadrilateral(static_cast<float>(attachFromX - 4),
                    static_cast<float>(track.bounds.getCentreY()),
                    static_cast<float>(attachFromX),
                    static_cast<float>(track.bounds.getCentreY() - 4),
                    static_cast<float>(attachFromX + 4),
                    static_cast<float>(track.bounds.getCentreY()),
                    static_cast<float>(attachFromX),
                    static_cast<float>(track.bounds.getCentreY() + 4));
                g.fillPath(diamond);
            ...
            if (track.mixTrack->attachTo.count() > 0)
            {
                const double attachToTime = std::chrono::duration<double>(track.audioStartTime + track.mixTrack->attachTo).count();
                const int attachToX =
                    track.bounds.getX() + static_cast<int>((attachToTime - std::chrono::duration<double>(track.componentStartTime).count()) * pixelsPerSecond_);

                // Draw attach point as a vertical line with circle
                g.drawVerticalLine(attachToX, static_cast<float>(track.bounds.getY()), static_cast<float>(track.bounds.getBottom()));

                // Draw circle marker
                g.fillEllipse(static_cast<float>(attachToX - 4), static_cast<float>(track.bounds.getCentreY() - 4), 8.0f, 8.0f);
            }
        }
    }
        ...
        if (pixelsPerGridLine > 5) // Only draw if grid lines are spaced enough
        {
            ...
            for (int x = firstGridPixel; x <= clipBounds.getRight(); x += pixelsPerGridLine)
            {
                ...
                if (isMajorLine)
                {
                    ...
                    // Ruler tick
                    g.drawLine(static_cast<float>(x), static_cast<float>(rulerHeight - 10), static_cast<float>(x), static_cast<float>(rulerHeight), 1.0f);
                ...
                else
                {
                    ...
                    // Ruler tick
                    g.setColour(juce::Colours::grey.withAlpha(0.5f));
                    g.drawLine(static_cast<float>(x), static_cast<float>(rulerHeight - 5), static_cast<float>(x), static_cast<float>(rulerHeight), 0.5f);
                }
            }
        }
    }
    ...
    void VirtualTimelineComponent::paintPlayhead(juce::Graphics &g)
    {
        ...
        if (playheadSeconds_ >= 0.0)
        {
            ...
            if (playheadX >= 0 && playheadX < getWidth())
            {
                ...
                juce::Path triangle;
                triangle.addTriangle(static_cast<float>(playheadX - 5), 0.0f, static_cast<float>(playheadX + 5), 0.0f, static_cast<float>(playheadX), 10.0f);
                g.fillPath(triangle);
            }
        }
    }
    ...
    std::vector<VirtualTimelineComponent::WaveformKey> VirtualTimelineComponent::getTileKeysForTrack(
        const TrackRenderData &track, const juce::Rectangle<int> &visibleArea) const
    {
    ...
    juce::Rectangle<int> VirtualTimelineComponent::getTileDestinationRect(const TrackRenderData &track, double tileStartTime, double tileEndTime) const
    {
        ...
        return juce::Rectangle<int>(startX, track.waveformBounds.getY(), endX - startX, track.waveformBounds.getHeight());
    }
