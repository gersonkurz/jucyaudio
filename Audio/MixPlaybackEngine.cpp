...
namespace jucyaudio
{
    namespace audio
    {
        ...
        //==============================================================================
        // MixPlaybackEngine implementation (ATTACH-based)
        //==============================================================================
        MixPlaybackEngine::MixPlaybackEngine()
        {
            m_formatManager.registerBasicFormats();
        ...
        bool MixPlaybackEngine::loadMix(MixProjectLoader *mixLoader)
        {
            ...
            // Clean up any existing mix
            unloadMixInternal();

            m_mixLoader = mixLoader;

            // Calculate track positions using ATTACH model
            calculateTrackPositions();

            // Calculate total duration using the mix loader's method
            m_totalDurationMs = m_mixLoader->calculateMixDuration();
            spdlog::info("MixPlaybackEngine: Loaded ATTACH-based mix with duration {}", durationToString(m_totalDurationMs));
        ...
        void MixPlaybackEngine::calculateTrackPositions()
        {
            ...
            for (size_t i = 0; i < mixTracks.size(); ++i)
            {
                ...
                m_trackPositions[mixTrack.trackId] = trackStart;
                previousTrackStart = trackStart;
                
                spdlog::debug("Track {} positioned at {} using ATTACH model", 
                            mixTrack.trackId, durationToString(trackStart));
            }
        }
        ...
        void MixPlaybackEngine::setPosition(Duration_t positionMs)
        {
            ...
            for (auto &source : m_trackSources)
            {
                // Get the calculated start position for this track
                auto it = m_trackPositions.find(source->trackId);
                if (it == m_trackPositions.end())
                    continue;
                
                Duration_t trackStartMs = it->second;
                juce::int64 trackStartSamples = static_cast<juce::int64>((trackStartMs.count() / 1000.0) * m_sampleRate);
        ...
        bool MixPlaybackEngine::prepareTrackSources()
        {
            ...
            for (const auto &mixTrack : mixTracks)
            {
                ...
                auto it = m_trackPositions.find(mixTrack.trackId);
                if (it != m_trackPositions.end())
                {
                    source->calculatedStartTime = it->second;
                }
            ...
            if (m_currentPositionSamples > 0)
            {
                ...
                for (auto &source : m_trackSources)
                {
                    Duration_t trackStartMs = source->calculatedStartTime;
                    juce::int64 trackStartSamples = static_cast<juce::int64>((trackStartMs.count() / 1000.0) * m_sampleRate);
        ...
        void MixPlaybackEngine::mixActiveTracksForBlock(juce::AudioBuffer<float> &buffer, juce::int64 startSample, int numSamples)
        {
            ...
            for (auto &source : m_trackSources)
            {
                ...
                // Calculate track timing in samples using ATTACH-based position
                juce::int64 trackStartSamples = static_cast<juce::int64>((source->calculatedStartTime.count() / 1000.0) * m_sampleRate);
                juce::int64 trackDurationSamples = static_cast<juce::int64>((source->trackInfo->duration.count() / 1000.0) * m_sampleRate);
                juce::int64 trackEndSamples = trackStartSamples + trackDurationSamples;
