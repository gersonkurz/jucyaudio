...
selftest config=default_build_type:
    ...
    cat "$root/selftest-results.txt" "$root/mixrecovery-results.txt" "$root/backup-results.txt" "$root/migration-results.txt" "$root/timeline-results.txt" "$root/foldercache-results.txt" "$root/transaction-results.txt" "$root/audioformat-results.txt"
